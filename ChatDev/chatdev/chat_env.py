...
class ChatEnv:
    ...
    @staticmethod
    def fix_module_not_found_error(error_response):
        try:
            error_data = error_response.json()
            if error_data["type"] == "error" and "ModuleNotFoundError" in error_data["error"]["message"]:
                module_match = re.search(r"No module named '(\S+)'", error_data["error"]["message"])
                if module_match:
            ...
            # Error response did not match the expected format, so we can't parse it
            pass
